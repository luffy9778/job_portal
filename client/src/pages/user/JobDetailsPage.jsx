        ...
        <h1 className="text-2xl font-bold capitalize">
          ...
          {jobData?.apllicationStatus && (
            <span className="px-5 text-sm text-gray-400">
              Applied(<span className={`${
    jobData?.apllicationStatus === "rejected"
      ? "text-red-500"
      : jobData?.apllicationStatus === "accepted"
      ? "text-green-500"
      : "text-blue-300"
  }`}>{jobData?.apllicationStatus}</span>)
            </span>
          )}
        </h1>
        {userData?.savedJobs?.includes(jobData?.job?._id)?
                <FontAwesomeIcon icon={solidBookmark} className="text-4xl text-orange-500 hover:scale-105"  onClick={()=>removeSavedJobs(jobData.job._id)}/>:
                <FontAwesomeIcon icon={regularBookmark} className="text-4xl text-orange-500 hover:scale-105" onClick={()=>saveJob(jobData.job._id)}/>}
        </div>
        ...
        <p className=" pl-10 font-medium capitalize">{jobData?.job?.recruiterId?.name}</p>
