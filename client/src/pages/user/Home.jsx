...
function Home() {
  ...
  const { query, setQuery, location, setLocation, handleSearch } =
  ...
  useEffect(() => {
    const fetchDummyJobs = async () => {
      try {
        const response = await axiosPrivate.get("/userJob/dummyJob");
        setDummyJobs(response.data);
      } catch (error) {
        console.error(error);
      }
    };
        ...
        <section className="text-center py-20 bg-gray-100">
          ...
          <div className="flex justify-center space-x-2 mb-4">
            <input
              ...
              className="border border-gray-400 p-2 rounded-lg w-1/3 focus:outline-orange-600"
              value={query}
            ...
            <input
              ...
              className="border border-gray-400 p-2 rounded-lg w-1/6 focus:outline-orange-600"
              value={location}
      ...
      <div className="font-sans text-center pt-10">
        ...
        <section id="jobs">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 p-6">
            {dummyJobs?.map(i=><JobCard key={i._id} job={i}/>)}
          </div>
        </section>
      </div>
      <Footer />
